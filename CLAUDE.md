...
node scripts/snapshot.mjs --days 2 --out snapshot/badminton.json   # what CI runs
```
...
**Pre-swept snapshot:** `.github/workflows/snapshot.yml` runs `scripts/snapshot.mjs` hourly and force-pushes the result to an orphan `data` branch, which `src/utils/snapshot.ts` fetches from raw.githubusercontent. `useSnapshotSeed` seeds `facilityCache` with it before the sweep starts, so the grid renders in about a second instead of the ~50s a cold sweep takes.

Two things make this safe rather than a stale-data trap. Entries are seeded with the snapshot's **own sweep time**, not `Date.now()`, so anything older than the cache TTL reads as stale — displayed immediately, then refetched live. And a missing, malformed, wrong-version or >12h-old snapshot parses to `null`, which puts the app back to exactly the behaviour it had before the snapshot existed. The branch is force-pushed as a single commit because a ~100 KB file rewritten 24×/day would add gigabytes to `main`'s history and redeploy the site every time. Badminton only, today + tomorrow; every other sport and date falls through to the live sweep. Do not treat a missed hour as a fault: GitHub delays scheduled runs under load and silently drops some altogether — an observed run fired at `08:07` for a `:17` cron and the next slot never fired at all. The 12h staleness window is what absorbs that, so widen the window rather than tightening the cron.

The sweeper is deliberately gentler than the app (5 at a time, 1s apart, 4 attempts with exponential backoff, 20s per-request timeout) because DBKL throttles *progressively* — the first CI run lost 10 venues on its first date and 26 on its second. Coverage is judged **per date**: a date below 70% is dropped rather than published, and the run only fails outright if no date clears the bar. A dropped date costs nothing, it just falls through to the live sweep.

**Caching strategy:**
- Location list is kept in module-level state (no re-fetch)
- Court availability goes through `src/utils/facilityCache.ts`, a module-level `Map` keyed by `sport|date|location_id` with a 10-minute TTL, read and written by **both** `useFacility` and `useAllFacilities` so the two never fetch the same venue twice. `readFacility` returns only fresh entries (used to decide what to fetch); `readStaleFacility` returns them at any age (used to decide what to show), which is what makes stale-while-revalidate work. `useAllFacilities` keys its groups by `location_id` rather than appending, so a stale venue already on screen is replaced in place when its fresh copy lands instead of being listed twice. Deliberately not persisted — stale bookings across sessions would be worse than a refetch. The refresh button calls `clearFacilityCache()` before bumping `refreshKey`; without that it would re-serve the same cached data and appear to do nothing
